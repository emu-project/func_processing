...
def submit_hpc_sbatch(command, wall_hours, mem_gig, num_proc, job_name, out_dir, env_input=False):
    ...
    env_input : bool, dict
        special environmental variables for certain jobs (fMRIprep)
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    sbatch_response = subprocess.Popen(sbatch_job, shell=True, stdout=subprocess.PIPE, env=env_input)
